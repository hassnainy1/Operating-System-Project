...
while true; do
	...
	echo " Roll Number already exist , Please  enter a unique roll number"
	else
	break
	fi
	done
...
echo "student added successfully! average : $avg  and grade: $grade "
...
update_student(){
 ...
 if grep -q "^$roll," $DATA_FILE; then
        ...
        echo "Enter New Subject 1 Grade:"

        read sub1

        echo "Enter New Subject 2 Grade:"

        read sub2

        echo "Enter New Subject 3 Grade:"

        read sub3
	sed -i"/^$roll,/d" $DATA_FILE
	echo "$roll,$name,$sub1,$sub2,$sub3" >> $DATA_FILE
...
view_all() {
    ...
    while IFS="|" read -r roll name s1 s2 s3 avg grade; do
        ...
        echo -e "$roll\t$name\t$sub1\t$sub2\t$sub3\t$avg\t$grade"
...
search_student() {
    ...
    if [ -n "$result" ]; then
	echo "Found"
	echo "Roll,Name,Subject1,Subject2,Subject3"
...
echo "All records deleted"
     ...
     echo "Operation sundoor cancelled"
fi 
}
